 ...
 */
	public function register($context, $local)
	{
		...
		if (!is_object($x))
		{
		    ...
		    if ($pw != $rpw)
		    {
			$local->addval('errmessage', 'The passwords do not match');
			$tpl = 'register.twig';
		    }
		    elseif (preg_match('/[^a-z0-9]/i', $login))
		    {
			$local->addval('errmessage', 'Your username can only contain letters and numbers');
			$tpl = 'register.twig';
		    }
		    elseif (!filter_var($email, FILTER_VALIDATE_EMAIL))
		    {
			$local->addval('errmessage', 'Please provide a valid email address');
			$tpl = 'register.twig';
		    }
		    else
		    {
			...
			$inst = $context->postpar('institution', '');
			if ($inst != '')
			{
			    $x->sharedInstitution[] = $context->load('institution', $inst);
			}
			R::store($x);
			$x->setpw($pw);
			$this->sendconfirm($x);
 ...
 */
	public function confirm($context, $local)
	{
		...
		{ # handle the form
		    ...
		    if (!is_object($user))
		    {
			$local->addval('errmessage', 'Sorry, there is no user with that name or email address.');
		    }
		    elseif ($user->confirm)
		    {
		...
		if (is_object($x))
		{
		    ...
		    {
			$local->addval('errmessage', 'Sorry, that code has expired! ');
		    }
		}
	    }
	    return $tpl;
 ...
 */
	public function forgot($context, $local)
	{
		...
		if (is_object($xc) && $xc->user_id == $user->getID())
		{
			...
			{
			    $local->addval('errmessage', 'Sorry, the passwords do not match! ');
			}
		    }
		    else
		    {
